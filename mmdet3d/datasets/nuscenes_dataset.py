...
class NuScenesDataset(Custom3DDataset):
            ...
            if self.use_sequence_group_flag:
                ...
                if not input_dict['start_of_sequence']:
                    ...
                    input_dict['curr_to_prev_ego_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                        self.data_infos[index], self.data_infos[index - 1],
                        "ego", "ego"))
                    # input_dict['prev_to_curr_ego_rt'] = torch.FloatTensor(nuscenes_get_rt_matrix(
                    #     self.data_infos[index - 1], self.data_infos[index],
                    #     "ego", "ego"))
                    # print("input_dict['prev_to_curr_ego_rt']:", input_dict['prev_to_curr_ego_rt'])
                else:
    ...
    def get_adj_info(self, info, index):
        info_adj_list = []
        for select_id in self.multi_adj_frame_id_cfg:
    ...
    def evaluate_occupancy(self, occ_results, runner=None, show_dir=None, save=False, **eval_kwargs):
        ...
        self.occ_eval_metrics = Metric_mIoU(
            ...
            use_image_mask=True)
        self.geo_occ_eval_metrics = Metric_mIoU(
            num_classes=2,
            use_lidar_mask=False,
            use_image_mask=True)
        ...
        for occ_pred_w_index in tqdm(occ_results):
            ...
            iou_occ_pred = np.zeros_like(occ_pred)
            iou_occ_pred[occ_pred==17] = 1
            iou_gt_semantics = np.zeros_like(gt_semantics)
            iou_gt_semantics[gt_semantics==17]=1
            self.geo_occ_eval_metrics.add_batch(
                iou_occ_pred,   # (Dx, Dy, Dz)
                iou_gt_semantics,   # (Dx, Dy, Dz)
                mask_lidar,     # (Dx, Dy, Dz)
        ...
        geo_res = self.geo_occ_eval_metrics.count_miou()
        if self.eval_fscore:
